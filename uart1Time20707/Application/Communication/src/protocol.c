...
//		uint8_t hourTime,minuTime,secondTime;
...
//								minuTime = UART_ReadChar(ptRxBuf,8);
//								secondTime = UART_ReadChar(ptRxBuf,9);
//							
//								/* 保存GPS信息 */
//								for( i=0; i<3; i++ )  /* 经度 */
//								{
//									gpsLatitude[ucNum++] = UART_ReadChar(ptRxBuf,10+i);
//								}
//								ucNum = 0;
//								for( i=0; i<3; i++ )  /*纬度*/
//								{
//									gpsLongitude[ucNum++] = UART_ReadChar(ptRxBuf,14+i);
//								}
//								ucNum = 0;
//								for( i=0; i<3; i++ )  /* 经度 */
//								{
//									gpsHeight[ucNum++] = UART_ReadChar(ptRxBuf,18+i);
//								}
//								ucNum = 0;
//								for( i=0; i<3; i++ )  /*纬度*/
//								{
//									gpsTime[ucNum++] = UART_ReadChar(ptRxBuf,22+i);
//								}
//							
//								Start_User_Timer(ptRxBuf->st_timer);
//								Update_User_Timer_Cnt(ptRxBuf->st_timer);
//                // 等待300ms，判断缓冲区是否充满设置的字长度  
//                if(((*ptRxBuf).ucBufCnt>=(ucCmdLength+2+2))&&(ptRxBuf->st_timer->ul_timer_cnt<=RECEIVE_TIM_CNT))         
//		      	    { 
